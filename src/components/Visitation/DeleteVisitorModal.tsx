...
import { VisitorType, SetState } from '../../../types/types';
...
const DeleteVisitorModal = ({
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await appAxios.delete('/visitor/' + selectedData?.id, {
        headers: {
      ...
      setAllData &&
        setAllData(allData?.filter((item: VisitorType) => item.id !== selectedData?.id));
    ...
    <CustomModal modalState={openModal} closeModal={closeModal} title='Delete Visitor'>
      <div>
        <p className='text-center md:text-left mb-10'>
          You are trying to delete this visitor: (
          {selectedData?.fname + ' ' + selectedData?.lname}). Are you sure you want to
          continue?
        </p>
