...
const AssignedSecondTimers = () => {
  ...
  React.useEffect(() => {
    const getData = async () => {
      try {
        ...
        const response = await appAxios.get(`/assigned-second-timer?page=${page}`, {
          headers: {
  ...
  return (
    <AppLayout pageTitle='Assigned Second Time Visitors' pageAction={<BackButton />}>
      {loading ? (
        <Loader />
      ) : data && data.length > 0 ? (
        <>
...
export default AssignedSecondTimers;
